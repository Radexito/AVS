...
class TestAudioCore(unittest.TestCase):
    ...
    def test_constants_exact(self) -> None:
        ...
        major_expected = np.array([6.35, 2.23, 3.48, 2.33, 4.38, 4.09, 2.52, 5.19, 2.39, 3.66, 2.29, 2.88], dtype=np.float32)
        minor_expected = np.array([6.33, 2.68, 3.52, 5.38, 2.60, 3.53, 2.54, 4.75, 3.98, 2.69, 3.34, 3.17], dtype=np.float32)
    ...
    def test_butter_lowpass_filter_paths(self) -> None:
        ...
        with patch.object(self.core, "butter", side_effect=_butter), patch.object(self.core, "lfilter", side_effect=_lfilter):
    ...
    def test_butter_lowpass_filter_empty(self) -> None:
        an = self.core.AudioAnalyzer(rate=48000)
        with patch.object(self.core, "butter", return_value=(np.array([1.0], dtype=np.float32), np.array([1.0], dtype=np.float32))), \
    ...
    def test_calculate_bpm_sets_value(self) -> None:
        ...
        expected = 120.0
        with patch.object(an, "butter_lowpass_filter", side_effect=lambda d, cutoff, fs, order=5: np.asarray(d, dtype=np.float32)), \
    ...
    def test_key_converges_minor(self) -> None:
        ...
        with patch.object(self.core.librosa.effects, "hpss", return_value=(np.ones(2048, dtype=np.float32), np.ones(2048, dtype=np.float32))), \
             patch.object(self.core.librosa.feature, "chroma_cqt", return_value=np.tile(minor_vec.reshape(12, 1), (1, 8))), \
             patch.object(self.core.librosa.util, "normalize", side_effect=lambda y, axis=None: np.asarray(y, dtype=np.float32)):
